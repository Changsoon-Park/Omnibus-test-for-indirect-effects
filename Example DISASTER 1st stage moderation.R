rm(list=ls())
D<-read.csv("https://raw.githubusercontent.com/Changsoon-Park/Omnibus-test-for-indirect-effects/main/disaster.csv")[,-1]
head(D)
names(D)=c("X","Y","M","J")
...
text(F.origin+1.2, 0.7,paste0("ASL=",round(p.value,4)))
############  end #########################
